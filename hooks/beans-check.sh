#!/bin/sh
# harness: block session end if code changed without a claimed beans issue
# Component: C6
# Event: Stop
# Matcher: (empty)
set -eu
...
# Only enforce in directories that use beans
if [ ! -d "$HOOK_CWD/.beans" ]; then
  exit 0
fi

# bn required for beans enforcement — graceful skip if not installed
...
# Code was changed — check for an in_progress beans issue
IN_PROGRESS=$(cd "$HOOK_CWD" && bn list --status=in_progress 2>/dev/null || true)
if [ -n "$IN_PROGRESS" ]; then
  exit 0
fi

# Block: code changes without a claimed issue
harness_error "code files modified but no beans issue claimed. Run: bn ready && bn update <id> --status=in_progress"
exit 2
